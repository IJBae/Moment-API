...
class ZenithKM
{
    ...
    {
        $redirect_code_uri = urlencode($this->redirect_uri);
    ...
    { //전체 광고계정 업데이트
        try {
            ...
            $msgs = [
                ...
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][카카오][광고계정] 수신 완료",
            ];
            ...
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][카카오][광고계정] 수신 오류 : " . $ex->getMessage(),
            ];
            $this->slack->sendMessage($msgs);
        }
    }
    ...
    { //전체 캠페인 업데이트
        try {
            ...
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][카카오][캠페인] 수신 완료",
            ];
            $this->slack->sendMessage($msgs);
            return $data;
        } catch (Exception $ex) {
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][카카오][캠페인] 수신 오류 : " . $ex->getMessage(),
            ];
            $this->slack->sendMessage($msgs);
        }
    }
    ...
    { //전체 광고그룹 업데이트
        try {
            ...
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][카카오][광고그룹] 수신 완료",
            ];
            $this->slack->sendMessage($msgs);
            return $result;
        } catch (Exception $ex) {
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][카카오][광고그룹] 수신 오류 : " . $ex->getMessage(),
            ];
            $this->slack->sendMessage($msgs);
        }
    }
    ...
    { //전체 소재 업데이트
        try {
            ...
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][카카오][소재] 수신 완료",
            ];
            $this->slack->sendMessage($msgs);
            return $result;
        } catch (Exception $ex) {
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][카카오][소재] 수신 오류 : " . $ex->getMessage(),
            ];
            $this->slack->sendMessage($msgs);
        }
    }
    ...
    public function updateBizform()
    {
        try {
            ...
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][카카오][잠재고객] 수신 완료",
            ];
            $this->slack->sendMessage($msgs);
        } catch (Exception $ex) {
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][카카오][잠재고객] 수신 오류 : " . $ex->getMessage(),
            ];
            $this->slack->sendMessage($msgs);
        }
    }
    ...
    { //소재별 보고서 BASIC 업데이트
        try {
            ...
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][카카오][보고서] {$datePreset} 수신 완료 (실행 시간: {$executionTime}초)",
            ];
            $this->slack->sendMessage($msgs);
            return $result;
        } catch (Exception $ex) {
            $msgs = [
                'channel' => $this->slackChannel,
                'text' => "[".date("Y-m-d H:i:s")."][" . getenv('MY_SERVER_NAME') . "][카카오][보고서] {$datePreset} 수신 오류 : " . $ex->getMessage(),
            ];
            $this->slack->sendMessage($msgs);
        }
    }
    ...
    */
    ...
    public function landingGroup($title, $landingUrl = '')
    {
        ...
        if (!empty($landingUrl) && preg_match("/hotblood\.co\.kr/i", $landingUrl)) {
            ...
            $_event_id = explode('/', $urls['path']);
            $event_id = @array_pop(preg_replace('/^([A-Z]{2})?([0-9]+)$/i', '$2', $_event_id));
            $site = $urls['qs']['site'] ?? '';
        } else {
